...
def calc_metrics(loader, label, label_onehot, model, criterion, method = "OpenMix_manifold", classnumber = 10):
    acc, softmax, correct, logit, conf_correct, conf_wrong, ece_correct, ece_wrong, nll_correct, nll_wrong = get_metric_values(
        loader, model, criterion, method=method, classnumber=classnumber)
    # aurc, eaurc
...
def calc_metrics_plot(loader, label, label_onehot, model, criterion, method = "OpenMix_manifold", classnumber = 10):
    acc, softmax, correct, logit, conf_correct, conf_wrong, ece_correct, ece_wrong, nll_correct, nll_wrong = get_metric_values(
        loader, model, criterion, method=method, classnumber=classnumber)
    # aurc, eaurc
    aurc, eaurc = calc_aurc_eaurc(softmax, correct)
...
def calc_ece(softmax, label, bins=15):
    ...
    softmax = torch.from_numpy(np.vstack(softmax))
...
def calc_nll_brier(softmax, logit, label, label_onehot):
    ...
    logit = torch.from_numpy(np.vstack(logit))
...
# Get softmax, logit
def get_metric_values(loader, model, criterion, method = "OpenMix_manifold", classnumber = 10):
    model.eval()
    with torch.no_grad():
        total_loss = 0.
        total_acc = 0.
        ...
        for input, target, idx in loader:
            ...
            output = model(input)
            if method  in ['classAug', 'OpenMix', 'OpenMix-CRL', 'OpenMix-flat', 'OpenMix_manifold']:
                output = output[:, :classnumber]
